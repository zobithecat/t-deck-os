...
> ⚠️ **Historical design doc — describes protocol v1.0** (the relay-layer
> introduction). The current, canonical spec is **[`PROTOCOL.md`](PROTOCOL.md)**
> — update that one; it carries the live protocol version (§12 changelog maps
> versions to changes). Some values here are stale (SF12 → now **SF9** since
> v1.1; PONG timing and hop-aware Range are v1.2, in PROTOCOL.md).
> Payloads are additionally classified **L0/L1/L2** since **v1.3** (2026-08-11:
> system lines carry a `!` prefix and never render as chat) — see PROTOCOL.md §5.
